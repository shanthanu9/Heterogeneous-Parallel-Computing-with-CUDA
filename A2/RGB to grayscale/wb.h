...
wbImage_t wbImportGray(const char* fName)
{
    std::ifstream inFile(fName, std::ios::binary);
    ...
    if (magic != "P5")
    {
        std::cerr << "Error reading image file " << fName << ". " << "Expecting 'P5' image format but got '" << magic << "'" << std::endl;
        inFile.close();
        std::exit(EXIT_FAILURE);
    }
    ...
    const int numElements = image.width * image.height ; //image.channels = 1;
...
{
    wbImage_t solnImage = wbImportGray(wbArg_getInputFile(args, args.argc - 2));
