...
wbImage_t wbImport(const char* fName)
{
    ...
    unsigned int* rawData = new unsigned int[numElements];

    //inFile.read(reinterpret_cast<int*>(rawData), numElements);
    for(int i=0; i<numElements; i++)
        inFile >> rawData[i];

    const int elementsRead = numElements; //static_cast<int>(inFile.gcount());
